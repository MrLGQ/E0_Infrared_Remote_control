  ...
  if(g_sEOmsg.EnOceanTelegramRxflags)
  {
    if(g_sEOmsg.EnOcean_RX_buf[PACKET_TYPE_SEQ] == PACKET_TYPES_REMOTE_MAN_COMMAND)
    {
     //GREEN_LED_ON();
      ...
      if(Fun==FUNCTION_NO_CTR_MSG)
      {//发送过来的Message是4字节  从buf【10】起  response 是 buf[12]
        GREEN_LED_ON();
//        if(g_sEOmsg.EnOcean_RX_buf[12]==0) flag=1;//OK
//        else if(g_sEOmsg.EnOcean_RX_buf[12]==1) flag=2;//没有红外信息
//        else if(g_sEOmsg.EnOcean_RX_buf[12]==2) flag=3;//命令错误 
//        else if(g_sEOmsg.EnOcean_RX_buf[12]==3) flag=4;//按键没有定义
        flag=(g_sEOmsg.EnOcean_RX_buf[12]+1);
      }
      delay_ms(500);
      GREEN_LED_OFF();
    }    
...
{//在这个里面对Enocean信息进行采集和处理
  ...
  if(g_sEOmsg.EnOceanTelegramRxflags)
  {
      if(g_sEOmsg.EnOcean_RX_buf[PACKET_TYPE_SEQ] == PACKET_TYPES_REMOTE_MAN_COMMAND)
      {
        ...
        //uint16_t datalen = g_sEOmsg.EnOcean_RX_buf[DATA_LENGTH_L_SEQ];
        uint16_t Fun = CONVERT_TO_INT(g_sEOmsg.EnOcean_RX_buf[REMOTE_MAN_FUN_NO_L_SEQ],g_sEOmsg.EnOcean_RX_buf[REMOTE_MAN_FUN_NO_H_SEQ]);
        if(Fun==FUNCTION_NO_WPS_MSG)
        { //获取目的地址  即上个设备发来的它自己的地址
          DestinationID1=CONVERT_TO_LONG(g_sEOmsg.EnOcean_RX_buf[17],g_sEOmsg.EnOcean_RX_buf[16],
