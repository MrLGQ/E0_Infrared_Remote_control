...
void CLOUSE_38K_PWM(void)
{
  ...
  gpio.GPIO_Pin = PWM_38K_PIN;
  gpio.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(PWM_38K_BASE, &gpio); //此处有疑问  为什么是高电平   
...
void Key_Event_Process(void)
{
  ...
  {
    Key_event_timeout++;
    if(Key_event_timeout < 100)
    {
      BLUE_LED_ON(); 
    }
    else if(Key_event_timeout >= 100&& Key_event_timeout < 200 ) //
    {
      BLUE_LED_TOGGLE();
    }
    else //Key_event_timeout >= 200 
    {
      BLUE_LED_ON();
     // GREEN_LED_ON();
    }
    SetEventTimeOut(KEY_EVENT,50);
  }
  else
  {
    if(Key_event_timeout < 100)
...
static void TIM2_Configuration(void)
{
  ...
  //定时时间T计算公式：
  //T=(TIM_Period+1)*(TIM_Prescaler+1)/TIMxCLK=(35999+1)*(1999+1)/72MHz=??us
  //单位是us
