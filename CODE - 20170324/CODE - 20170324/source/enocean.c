...
void USART1_IRQHandler(void)
{
  ...
  {	      
    ...
    while(USART_GetFlagStatus(USART1, USART_FLAG_TXE) != SET);//等待
    ...
    if(g_sEOmsg.EnOcean_RX_len < ENOCEAN_RX_MAX && (!g_sEOmsg.EnOceanTelegramRxflags))
    {
      ...
      if(!PacketHeader)
      {
        ...
        {
          if(g_sEOmsg.EnOcean_RX_len == PACKET_HEADER_LEN+2)//接收完报头了 4+sync +crc
          {
      ...
      //接收完报头部分后一直接收  直到抱头中计算出的数据长度等于接收到的实际长度
      //结束接收  中断标志置位
...
//测试用发送EO型 ir学习遥控器指令
// int uint8_t Eo_SendIr(void)
//{
//  uint8_t result = 1;
//  uint8_t buf[4];
//  buf[SYNC_BYTE_SEQ] = SYNC_BYTE;
//  buf[DATA_LENGTH_H_SEQ] = 0;
//  buf[DATA_LENGTH_L_SEQ] = 0;
//  buf[OPTION_LENGTH_SEQ] = 4;
//  buf[PACKET_TYPE_SEQ] = PACKET_TYPES_RADIO;
...
static uint8_t TCM310FReset(void)
{
  ...
  return result;//好处多多  return会跳出本次函数
}
...
{
  memset(&g_sEOmsg, 0, sizeof(g_sEOmsg));//结构体初始化  将其中所有的值都0

  memset(Device_GW_ID, 0xff, sizeof(Device_GW_ID));//设备地址全为ff  这是按照一个字节写入的  因此是ff 不是ffffffff
  
  if(TCM310FReset())return 1;
  delay_ms(50);  
  if(GetChipID())return 1;
  //if(FilterEnable(1))return 1;
  ReadCtrDevice(Device_GW_ID);//读取网管的chip ID
...
void Send_WPS_Msg(void)
{
  ...
  if(GetWorkMode() == Mode_WPS){//发送10次 配对信息
...
{//在这个里面对Enocean信息进行采集和处理
  ...
  if(g_sEOmsg.EnOceanTelegramRxflags)
  {
    if(g_sEOmsg.EnOcean_RX_buf[PACKET_TYPE_SEQ] == PACKET_TYPES_REMOTE_MAN_COMMAND)
    {
      ...
      switch(Fun)
      {
      case FUNCTION_NO_WPS_MSG: break;//1
      case FUNCTION_NO_WPS_RSP_MSG:{
        if(datalen == 4 && GetWorkMode() == Mode_WPS){
          ...
          //将回复过来的sourceID存储  发送Msg时候用做Destination
          g_sEOmsg.SourceID = CONVERT_TO_LONG(g_sEOmsg.EnOcean_RX_buf[17],g_sEOmsg.EnOcean_RX_buf[16],
                                              g_sEOmsg.EnOcean_RX_buf[15],g_sEOmsg.EnOcean_RX_buf[14]);
          //if(DestinationID == g_sEOmsg.TCM310FChipID && PollCtrDevice(g_sEOmsg.SourceID))
          //这个是用来判断地址是不是已经有了的程序 需要添加
           if(DestinationID == g_sEOmsg.TCM310FChipID)
          {
            if(PollCtrDevice(g_sEOmsg.SourceID))
            {
            //没有这个地址 给添加进去
            AddDevice(g_sEOmsg.SourceID);
            SetWorkMode(Mode_Normal);//蓝灯一直亮 说明没有进到这个程序里面来
            }
            else
            {//有这个地址  闪烁两下  然后关掉
              BLUE_LED_TOGGLE();delay_ms(400);BLUE_LED_TOGGLE();delay_ms(400);BLUE_LED_TOGGLE();delay_ms(400);
              SetWorkMode(Mode_Normal);
            }
          }
        }
      ...
      case FUNCTION_NO_CTR_MSG:{
        if(datalen == 8){
          memset(ir_data, 0, sizeof(ir_data));//清零数组中的值
          ...
          if(DestinationID == g_sEOmsg.TCM310FChipID/* && !PollCtrDevice(SourceID)*/)
          {
            ...
            //IR_Send(code);  
              memset(ir_data,0,4);//清除数组中的值
              ir_data[data_cmd]=g_sEOmsg.EnOcean_RX_buf[10];
              ir_data[data_data]=g_sEOmsg.EnOcean_RX_buf[11];
              ir_data[data_response]=g_sEOmsg.EnOcean_RX_buf[12];
              ir_data[data_null]=g_sEOmsg.EnOcean_RX_buf[13];
              if(ir_data[data_cmd]==1)
              {
                SetEvent(IR_STUDY_EVENT);//学习开始  命令为1 表示学习            
              }
              else if(ir_data[data_cmd]==2)
              {//cmd为2 表示发送命令
                //SetEvent();//命令为2 表示发送
              ...
              {//命令错误
                ir_data[data_response]=2;//2 表示命令错误    
